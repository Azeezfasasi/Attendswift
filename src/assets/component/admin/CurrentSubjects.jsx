            ...
            {filteredData.length > 0 ? (
              filteredData.map((item, index) => (
                <tr key={index} className="border-t">
                  ...
                  <td className="p-3">
                    {editingSubject === item.id ? (
                      <button onClick={() => handleSaveEdit(item.id)} className="text-green-600 hover:underline mr-4">Save</button>
                    ) : (
                      <button onClick={() => handleEditClick(item)} className="text-blue-600 hover:underline cursor-pointer mr-4">Edit</button>
                    )}
                    <button onClick={() => deleteSubject(item.id)} className="text-red-600 hover:underline cursor-pointer">Remove</button>
                  </td>
                </tr>
              ))
            ) : (
